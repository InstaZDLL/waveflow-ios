...
Swift Testing (`@Test` / `#expect`), dans `WaveFlowTests/`. Trois suites, toutes exécutables depuis
Linux : `GroupingTests`, `DurationFormatTests`, `TagNormalizationTests` (les helpers de `Song.swift`
sur lesquels reposent les identifiants de regroupement).
